...
function NostalgyImportRules() {
  var rules = NostalgyMkPrefStr();
  var s = prompt(
    "You can paste here a set of rules created with the 'Export Rules' button.",
    rules
  );
  if (!s) return;

  s = s.replace(/([\x00-\x20>])/g, function(a, b) {
    return "";
  });
  if (
    confirm(
    ...
    while (nostalgy_gList.getRowCount() > 0) nostalgy_gList.removeItemAt(0);
    var i;
    for (i = 0; i < r.length; i++) {
      NostalgyCreateItem(r[i]);
    }
  }
}
...
function NostalgyRuleOfItem(item) {
  ...
  return {
    folder: item.childNodes.item(3).getAttribute("value"),
...
function NostalyStrOfRule(rule) {
  return (
    ...
    "," +
    "contains:" +
    NostalgyQuote(rule.contains) +
    "," +
    "under:" +
    NostalgyQuote(rule.under) +
    "," +
    "folder:" +
    NostalgyQuote(rule.folder) +
    "}"
  );
}
...
function NostalgyMkPrefStr() {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    res =
      res +
      NostalyStrOfRule(NostalgyRuleOfItem(nostalgy_gList.getItemAtIndex(i)));
  }
  return "[" + res + "]";
}
...
function NostalgyEditRule(rule, accept) {
  window.openDialog(
    "chrome://nostalgy/content/edit_rule.xul",
    "_blank",
...
function NostalgyDoEditItem(item) {
  if (item) {
    NostalgyEditRule(NostalgyRuleOfItem(item), function(rule) {
      NostalgySetItem(item, rule);
    });
  }
}
...
function onNostalgyAcceptChanges() {
  ...
  try {
    prefs.setIntPref(
      "extensions.nostalgy.number_of_recent_folders",
      0 + NostalgyEBI("number_of_recent_folders").value
    );
  } catch (exn) {
    NostalgyDebug(exn);
  }
  try {
    prefs.setIntPref(
      "extensions.nostalgy.predict_max_addresses_to_update",
      0 + NostalgyEBI("predict_max_addresses_to_update").value
    );
  } catch (exn) {
    NostalgyDebug(exn);
  }
  ...
  for (var i in nostalgy_keys) {
    ...
    prefs.setCharPref(
      nostalgy_kKeysPrefs + nostalgy_keys[i][0],
  ...
  for (var i = 0; i < e.length; i++)
    if (e[i].id.substr(0, 5) == "key__") {
      ...
      prefs.setCharPref(
        nostalgy_kCustomActionsPrefs + id,
        e[i].previousSibling.value
      );
    }
...
function NostalgyDoDelete() {
  ...
  if (idx >= 0) {
    ...
    if (nostalgy_gList.getRowCount() <= idx) {
      idx = nostalgy_gList.getRowCount() - 1;
    }
...
function onNostalgyLoad() {
  document.addEventListener("dialogaccept", event => {
    onNostalgyAcceptChanges();
  });
  NostalgyFolderSelectionBoxes();
  document.addEventListener("dialogextra2", event => {
    openDialog(
      "chrome://nostalgy/content/about.xul",
      "about_nostalgy",
      "resizable"
    );
  });
  ...
  NostalgyEBI("number_of_recent_folders").value = NostalgyGetIntPref(
    prefs,
    "number_of_recent_folders",
    5
  );
  NostalgyEBI("predict_max_addresses_to_update").value = NostalgyGetIntPref(
    prefs,
  ...
  for (var i = 0; i < nostalgy_keys.length; i++) {
    ...
    nostalgy_key_rows.appendChild(
      NostalgyCreateKeyRow(nostalgy_keys[i][0], nostalgy_keys[i][1], v)
    );
  }
  ...
  for (var i in a) {
    ...
    if (id.substr(0, 1) == "_") {
      ...
      try {
        ...
        nostalgy_key_rows.appendChild(
          NostalgyCreateKeyRow(id, cmd, prefs.getCharPref(a[i]))
        );
      } catch (ex) {}
    }
  }
...
function onNostalgyKeyPress(ev) {
  ...
  if (!nostalgy_wait_key && (ev.keyCode == 46 || ev.keyCode == 8))
    NostalgyDoDelete();
  // should only to that in the relevant tab
    ...
    nostalgy_wait_key.value = nostalgy_wait_key_old;
    nostalgy_wait_key = null;
  } else if (nostalgy_wait_key) {
    /* && (ev.keyCode != 13 || ev.ctrlKey || ev.altKey)) */ NostalgyRecognize(
      ev,
      nostalgy_wait_key
    );
    nostalgy_wait_key = null;
...
function NostalgySelectFolder() {
  if (nostalgy_folder_select.value != "") {
    ...
    if (folder) {
      ...
      nostalgy_key_rows.appendChild(
        NostalgyCreateKeyRow(
          "_" + nostalgy_max_custom,
          cmd + " -> " + name,
          "(disabled)"
        )
      );
...
function NostalgyDoRestart() {
  Services.startup.quit(
    Ci.nsIAppStartup.eRestart | Ci.nsIAppStartup.eForceQuit
  );
}
