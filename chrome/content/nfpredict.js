...
var nostalgy_CreateTablesQuery1 =
  "CREATE TABLE IF NOT EXISTS addresses (id INTEGER PRIMARY KEY AUTOINCREMENT, address TEXT, count INTEGER)";
var nostalgy_CreateTablesQuery2 =
  "CREATE TABLE IF NOT EXISTS folders (id INTEGER PRIMARY KEY AUTOINCREMENT, folder TEXT)";
var nostalgy_CreateTablesQuery3 =
  "CREATE TABLE IF NOT EXISTS probabilities (id INTEGER PRIMARY KEY AUTOINCREMENT, address_id INTEGER, folder_id INTEGER, probability REAL, count INTEGER)";
var nostalgy_CreateIndexesQuery1 =
  "CREATE INDEX IF NOT EXISTS address_index on addresses(address)";
var nostalgy_CreateIndexesQuery2 =
  "CREATE INDEX IF NOT EXISTS folder_index on folders(folder)";
var nostalgy_CreateIndexesQuery3 =
  "CREATE INDEX IF NOT EXISTS probabilities_index on probabilities(address_id,folder_id)";

var nostalgy_PredictQueryA =
  "SELECT avg(probabilities.count*100/addresses.count) as prob,folder FROM addresses,folders,probabilities " +
...
var nostalgy_AddressInsert =
  "INSERT INTO addresses(address,count) VALUES(?1,0);";
...
var nostalgy_ProbabilityInsert =
  "INSERT INTO probabilities(address_id , folder_id , probability , count) VALUES(?1,?2,?3,?4);";
var nostalgy_UpdateProbabilityCount =
  "UPDATE probabilities SET count=?2, probability=?3 WHERE id=?1;";

var nostalgy_CountsQuery =
  "SELECT distinct addresses.id as address_id, addresses.count as address_count, probabilities.id as probability_id, probabilities.count as probability_count " +
...
var nostalgy_CountsQueryAll =
  "SELECT distinct addresses.id as address_id, addresses.count as address_count, probabilities.id as probability_id, probabilities.count as probability_count FROM addresses, probabilities WHERE addresses.id=probabilities.address_id;";
...
var NostalgyPredict = {
  ...
  init: function() {
    ...
    for (var i = 1; i <= 20; i++) {
      try {
        this.emails[
          prefs.getCharPref("mail.identity.id" + i + ".useremail")
        ] = 1;
      } catch (ex) {}
    }
      ...
      if (gDBView != null) {
        ...
        for (var i = 0; i < aAdresses.length; i++) {
          if (aAdresses[i].match(email_re) && this.keep_email(aAdresses[i])) {
            if (addrs.length != 0) addrs = addrs + ", ";
            addrs = addrs + "'" + aAdresses[i] + "'";
          }
        }
        //                NostalgyDebug("addrs = " + addrs);
        try {
          var nostalgy_Array1 = nostalgy_sqlite.select(
            this.getDBFile(),
            nostalgy_PredictQueryA +
              addrs +
              nostalgy_PredictQueryB +
              numPredictions +
              ";"
          );
  ...
  find_folder_id: function find_folder_id(folder) {
    return this.find_generic_id(
      folder,
      nostalgy_FolderInsert,
      nostalgy_FolderQuery
    );
  },
  ...
  find_address_id: function(address) {
    return this.find_generic_id(
      address,
      nostalgy_AddressInsert,
      nostalgy_AddressQuery
    );
  },
  ...
  update_probabilites: function() {
    ...
    for (var j = 0; j < nostalgy_Array1.length; j++) {
      ...
      nostalgy_sqlite.cmd(
        this.getDBFile(),
        nostalgy_UpdateProbabilityCount,
        nostalgy_Array1[j]["probability_id"],
        prob_count,
        prob
      );
    }
  ...
  update_folder: function(nsiFolder) {
    if (nostalgy_completion_options.use_statistical_prediction == false) return;
    ...
    try {
      ...
      NostalgyDebug("Cannot get header for first message:" + ex);
      return;
    }
    ...
    try {
      maxAddresses = NostalgyGetIntPref(
        "predict_max_addresses_to_update",
        maxAdresses
      );
    } catch (ex) {}
    for (var i = 0; i < aAdresses.length; i++) {
      ...
      if (
        aAdresses[i].match(email_re) &&
        this.keep_email(aAdresses[i]) &&
        maxAddresses > 0
      ) {
        maxAddresses--;
        ...
        if (nostalgy_Array1.length == 0) {
          ...
          var nostalgy_Array2 = nostalgy_sqlite.select(
            this.getDBFile(),
            nostalgy_GetAddressCount,
            address_id
          );
          if (nostalgy_Array2.length != 0) {
          ...
          for (var j = 0; j < nostalgy_Array1.length; j++) {
            ...
            var prob_count =
              1 + parseInt(nostalgy_Array1[j]["probability_count"]);
            var prob = parseFloat(prob_count) / parseFloat(addr_count);
