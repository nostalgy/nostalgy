...
function NostalgySaveRecentFolder(recent) {
  NostalgyPrefBranch().setCharPref(
    "extensions.nostalgy.recent_folders",
    recent.toSource()
  );
}
...
function NostalgyInstallRecentFolders() {
  var s = "";
  try {
    s = NostalgyPrefBranch().getCharPref("extensions.nostalgy.recent_folders");
  } catch (ex) {
    return;
  }
...
function NostalgyMayLowerCase(s) {
  if (!nostalgy_completion_options.match_case_sensitive) return s.toLowerCase();
  else return s;
}
...
function NostalgyMayMatchOnlyPrefix(s, reg) {
  ...
  else return NostalgyMayLowerCase(s).match(reg);
}
...
function NostalgyFolderMatch(f, reg) {
  if (nostalgy_completion_options.match_only_folder_name) {
    return (
      NostalgyMayMatchOnlyPrefix(NostalgyPrettyName(f), reg) ||
...
function NostalgyGetAutoCompleteValuesFunction(box) {
  return function NostalgyGetAutoCompleteValues(text) {
    ...
    var f = function(folder) {
      add_folder(NostalgyFolderName(folder));
    };
    ...
    if (text == "") {
      ...
      if (nostalgy_completion_options.use_statistical_prediction) {
        try {
          predictedFolders = NostalgyPredict.predict_folder(
            nostalgy_recent_folders_max_size
          );
        } catch (ex) {}
      ...
      for (var j = 0; j < nostalgy_recent_folders.length; j++) {
        var found = 0;
        if (
          nostalgy_completion_options.use_statistical_prediction &&
          predictedFolders != null &&
          predictedFolders.length > 0
        )
          for (var i = 0; i < predictedFolders.length; i++)
            if (
              NostalgyFolderName(predictedFolders[i]) ==
              nostalgy_recent_folders[j]
            )
              found = 1;
...
function NostalgyFolderSelectionBox(box) {
  ...
  box.searchParam = NostalgyAutocompleteComponent().attachGetValuesFunction(
    NostalgyGetAutoCompleteValuesFunction(box)
  );

  box.onkeypress = function(event) {
    if (
      event.keyCode == KeyEvent.DOM_VK_TAB &&
      box.getAttribute("normaltab") != "true"
    ) {
  ...
  NostalgyIterateFolders(function(f) {
    ...
    if (n.search(rexp) == 0) {
      ...
      if (nb == 1) {
        ...
        ret = NostalgyLongestCommonPrefix(ret, n);
      }
    }
  });
  if (ret) {
...
function NostalgyFirstCompletion(uri) {
  var ret = null;
  NostalgyIterateMatches(uri, false, function(f) {
    ret = f;
    throw 0;
  });
  return ret;
...
function NostalgyFindFolderExact(uri) {
  ...
  try {
    NostalgyIterateFoldersAllServers(function(folder) {
      if (NostalgyMayLowerCase(NostalgyFullFolderName(folder)) == u) {
        ret = folder;
        throw 0;
      }
    });
...
function NostalgyFindFolderCompleted(uri) {
  ...
  try {
    NostalgyIterateFoldersAllServers(function(folder) {
      if (NostalgyMayLowerCase(NostalgyFolderName(folder)) == u) {
        ret = folder;
        throw 0;
      }
    });
...
function NostalgyFindFolderCropped(uri) {
  ...
  try {
    NostalgyIterateFolders(function(folder) {
      if (NostalgyCrop(NostalgyFolderName(folder)) == uri) {
        ret = folder;
        throw 0;
      }
    });
...
function NostalgyIterateFoldersAllServers(f) {
  ...
  var amService = Components.classes[
  ...
  if (servers.length) nservers = servers.length;
  /* TB >= 20 */ else if (servers.Count) nservers = servers.Count();
  /* TB < 20 */ else alert("NOSTALGY: cannot determine server count");

  for (i = 0; i < nservers; i++) {
    var server;
    if (servers.GetElementAt)
      /* TB < 20 */
      server = servers
        .GetElementAt(i)
        .QueryInterface(Components.interfaces.nsIMsgIncomingServer);
    else if (servers.queryElementAt)
      /* TB >= 20 */
      server = servers.queryElementAt(
        i,
...
function NostalgyIterateSubfolders(folder, f) {
  if (
    ...
    (folder.canFileMessages || !nostalgy_search_folder_options.require_file)
  ) {
    try {
      f(folder);
    } catch (ex) {
  ...
  if (folder.subFolders) {
    ...
    while (subfolders.hasMoreElements()) {
      ...
      if (nostalgy_completion_options.sort_folders) {
        ...
        NostalgyIterateSubfolders(subfolder, f);
      }
    }
  } else {
    // TB < 3.0
...
function NostalgyIterateMatches(uri, shell, f) {
  ...
  if (shell) {
    ...
    try {
      NostalgyIterateFolders(function(folder) {
        if (NostalgyFolderMatch(folder, rexp)) {
          f(folder);
        }
      });
    } catch (ex) {}
