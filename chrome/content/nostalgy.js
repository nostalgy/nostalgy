...
var NostalgyRules = {
  register: function() {
    ...
    this._branch2 = this._branch.QueryInterface(
    ...
    try {
      nostalgy_recent_folders_max_size = this._branch.getIntPref(
  ...
  register_keys: function() {
    ...
    for (var i in nostalgy_keys) {
      ...
      nostalgy_default_label =
        "save (" + sSave + ") copy (" + sCopy + ") go (" + sGo + ")";
      if (nostalgy_label) nostalgy_label.label = nostalgy_default_label;
    }
  ...
  apply: function(sender, subject, recipients, cfolder) {
    ...
    for (i = 0; i < rules.length && !folder; i++) {
      var r = rules[i];
      if (
        ((r.subject && NostalgyMatchContains(subject, r.contains)) ||
...
function onNostalgyResize() {
  if (nostalgy_label)
    nostalgy_label.parentNode.maxWidth =
      (document.documentElement.clientWidth * 6) / 10;
}

var NostalgyFolderListener = {
  OnItemAdded: function(parentItem, item) {
    try {
      var i = item.QueryInterface(Components.interfaces.nsIMsgFolder);
      ClearNostalgyCache();
    } catch (ex) {}
  },
...
function NostalgyMailSession() {
  var mSession = Components.classes[
...
function onNostalgyLoad() {
  ...
  if (!nostalgy_in_message_window) {
    NostalgyEBI("threadTree").addEventListener(
      "select",
      NostalgyDefLabel,
      false
    );
  ...
  if (mSession)
    mSession.AddFolderListener(
      NostalgyFolderListener,
...
function NostalgyDefLabel() {
  ...
  if (nostalgy_gsuggest_folder) {
    nostalgy_label.label =
      nostalgy_default_label +
      " [+Shift: => " +
      NostalgyFolderName(nostalgy_gsuggest_folder) +
      "]";
  } else {
    nostalgy_label.label = nostalgy_default_label;
  }
...
function NostalgyCollapseFolderPane() {
  ...
  if (window.MsgToggleFolderPane) {
    ...
    MsgToggleSplitter("gray_vertical_splitter");
    return true;
  } else if (fp) {
...
function NostalgyCmd(lab, cmd, require_file) {
  ...
  if (!nostalgy_focus_saved) {
    nostalgy_focus_saved = NostalgyEBI("messagepane").contentWindow;
  }
...
function NostalgyShowRecentFoldersList() {
  ...
  if (box.controller) {
    // Toolkit
    listener = box.controller.QueryInterface(
  ...
  NostalgyAutocompleteComponent().startSearch(
    "",
    box.searchParam,
    null,
    listener
...
function NostalgyCreateTag(name) {
  var tagService = Components.classes[
...
function NostalgyRunCommand() {
  ...
  if (f) {
    ...
    if (s.substr(0, 1) == ":") {
      if (s == ":" || s == "::") return;
      var name;
      if (s.substr(s.length - 1, 1) == ":") name = s.substr(1, s.length - 2);
      else name = s.substr(1, s.length - 1);
      ...
      if (i <= 0) {
        ...
        if (i == 0) name = s.substr(1, s.length - 1);
        else name = s;
      } else {
...
function NostalgyMailAuthorName() {
  return NostalgyHeaderParser.get_address(
    gDBView.hdrForFirstSelectedMessage.author
  );
}

function NostalgyMailRecipName() {
  return NostalgyHeaderParser.get_address(
    gDBView.hdrForFirstSelectedMessage.recipients
  );
}
...
function NostalgySuggest() {
  ...
  try {
    if (folder)
      r = NostalgyRules.apply(
        NostalgyMailAuthor(),
        NostalgyMailSubject(),
        NostalgyMailRecipients(),
        folder
      );
    if (r) {
      return r;
...
function NostalgyShowFolder(folder) {
  ...
  if (window.CycleFolderView) {
    ...
    try {
      ...
      savedFolderView = window.gFolderTreeView.modeNames.indexOf(
        window.gFolderTreeView.mode
      );
    } catch (ex) {
      totry = window.gFolderTreeView._modeNames.length;
      savedFolderView = window.gFolderTreeView._modeNames.indexOf(
        window.gFolderTreeView.mode
      );
    }
  ...
  if (window.gFolderTreeView) {
    ...
    try {
      while (totry > 0) {
        ...
        if (window.gFolderTreeView.getIndexOfFolder(folder)) totry = 0;
        else {
          totry--;
    ...
    while (totry > 0) {
      try {
        ...
        totry--;
        CycleFolderView(true);
      }
    }
    if (window.CycleFolderView) {
      loadFolderView(savedFolderView);
    }
  }
  setTimeout(function() {
    SetFocusThreadPane();
...
function NostalgyMoveToFolder(folder) {
  ...
  if (folder.tag) NostalgyToggleMessageTag(folder);
    ...
    gDBView.doCommandWithFolder(
      Components.interfaces.nsMsgViewCommandType.moveMessages,
      folder
    );
  }
  return true;
...
function NostalgyCopyToFolder(folder) {
  ...
  if (folder.tag) NostalgyToggleMessageTag(folder);
    ...
    NostalgyPredict.update_folder(folder);
    gDBView.doCommandWithFolder(
      Components.interfaces.nsMsgViewCommandType.copyMessages,
      folder
    );
  }
  return true;
...
function NostalgyScrollMsg(d) {
  if (NostalgyIsThreadPaneFocused()) {
    var b = NostalgyEBI("messagepane").contentDocument.getElementsByTagName(
      "body"
    )[0];
    if (b) {
      b.scrollTop += d;
...
function NostalgyEscape() {
  ...
  setTimeout(function() {
    if (NostalgyEscapePressed == i) NostalgyEscapePressed = 0;
  }, 300);
...
function NostalgySearchSender() {
  ...
  if (!input) {
    alert(
      "Nostalgy error:\nCannot perform this action when Quick Search is not enabled"
    );
    return false;
  }
  try {
    ...
    if (input.value != nostalgy_last_cycle_restrict_value)
      nostalgy_last_cycle_restrict = 0;
    ...
    if (name != "" && nostalgy_last_cycle_restrict == 1) {
      ...
      else if (window.kQuickSearchSender) input.searchMode = kQuickSearchSender;
      else if (window.kQuickSearchFrom) input.searchMode = kQuickSearchFrom;
...
function onNostalgyKeyPressCapture(ev) {
  ...
  var focused = "";
  try {
    focused = document.commandDispatcher.focusedElement.nodeName;
  } catch (ex) {}
  if (!nostalgy_statusBar.hidden && focused != "html:input") {
    // ugly hack: it takes some time for the folderBox to be focused
    if (ev.charCode) {
      nostalgy_folderBox.value =
        nostalgy_folderBox.value + String.fromCharCode(ev.charCode);
    }
...
function onNostalgyKeyPress(ev) {
  ...
  if (NostalgyEscapePressed >= 1) {
    if (!nostalgy_in_message_window && ev.charCode == 109) {
      // M
      NostalgyFocusMessagePane();
      NostalgyStopEvent(ev);
    } else if (!nostalgy_in_message_window && ev.charCode == 102) {
      // F
  ...
  if (
    ev.charCode &&
    (ev.originalTarget.localName.toLowerCase() == "input" ||
...
function NostalgyParseCommand(k) {
  if (k.indexOf("JS:") == 0) return eval(k.substr(3, k.length - 3));

  var spl = k.match(/(.*) -> (.*)/);
  var folder = NostalgyFindFolderExact(spl[2]);
  if (!folder) {
    alert("Cannot find folder " + spl[2]);
    return;
  }
  switch (spl[1]) {
    case "Go":
      return NostalgyShowFolder(folder);
    case "Save":
      return NostalgyMoveToFolder(folder);
    case "Copy":
      return NostalgyCopyToFolder(folder);
    case "SaveGo":
      return NostalgyMoveToFolderAndGo(folder);
    default:
      alert("Unknown command " + spl[1]);
      return;
  }
}
